        ...
        inline_keyboard=[
            [
                InlineKeyboardButton(
                    text="❌ Отмена",
                    callback_data="cancel_tariff_question"
                )
            ]
        ]
    )
...
async def process_tariff_question(message: Message, state: FSMContext):
    ...
    try:
        ...
        tariff_context = f"Информация о тарифах для территории {territory_id}:\n{str(tariff_info)}"
        ...
        if ai_response:
            ...
            keyboard = InlineKeyboardMarkup(
                inline_keyboard=[
                    ...
                    [
                        InlineKeyboardButton(
                            text="💬 Обычный режим",
                            callback_data="switch_to_general"
                        )
                    ],
                    [
                        InlineKeyboardButton(
            ...
            logger.info(f"Успешно обработан тарифный запрос пользователя {message.from_user.id}")
        else:
            await message.answer("⚠️ Произошла ошибка при обработке вашего вопроса. Попробуйте позже.")
...
async def end_conversation(callback, state: FSMContext):
    ...
    await callback.message.edit_text(
        "✅ Разговор завершен. Спасибо за обращение!"
    )
